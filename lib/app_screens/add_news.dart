...
class _AddNewsState extends State<AddNews> {
  ...
  Future<void> _showStatusDialog(String status) async {
    await showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
            'Your post is $status. You can see all your uploaded news in your profile settings. ${status == 'pending approval from admins' ? 'You can proceed further with thumbnail upload and news publish once you get an admin approval. Please be patient!' : ''} '),
        actions: [
          TextButton(
            onPressed: () {
                ...
                actions: [
                  IconButton(
                    ...
                    onPressed: () async {
                      ...
                      await _saveAndShowDialog(
                          'pending', 'pending approval from admins');
                    },
                  ),
                ],
              ),
          ...
          actions: [
            ElevatedButton(
              onPressed: (_titleController.text.isNotEmpty &&
                      (_thumbnail != null || _thumbnailUrl != null))
                  ? () async {
                          ...
                          children: [
                            Expanded(
                              child: _thumbnail != null
                                  ? Image.network(_thumbnail!.path)
                                  : _thumbnailUrl != null
                                      ? FutureBuilder<Uint8List>(
                                          future: getImageData(_thumbnailPath!),
  ...
  Widget _embedBuilder(BuildContext context, EmbedNode node) {
    if (node.value.type == 'icon') {
      ...
      final iconName =
          data['iconName'] ?? 'home'; // Default to 'home' if not specified
      final icon = icons[iconName] ??
          Icons.home; // Use home icon if iconName is not found
